...
// 싱글스레드 : 하나의 프로세스에 하나의 스레드가 일처리 하는 것 (day01~ day18)
// java에서 스레드 생성법 
// ★자바에서는 start()를 호출하여 run()가 실행 되면 스레드 처리(실행) 됐다고 함★
// 1. Thread 클래스 상속 받기 : start(), run() 메서드가 모두 있따.
// 2. Runnable 인퍼페이스 구현 : run()만 존재 
...
public class Ex01_Thread {
	...
	} //여기까지 다 끝나면 밑에 main play 로 돌아간다
	...
	public static void main(String[] args) {
		// 지금은 싱글스레드(즉, 하나의 스레드가 모든일을 다 처리한다.)

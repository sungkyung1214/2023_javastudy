...
public class Ex01_class {
	...
	// 					  기능 = 동작 =멤버 메서드
	//					  생성자 = 클래스가 객체로 만들어질 때 한번 호출 되서 데이터를 초기화 할 수 있는 기능을 가졌다.
	...
	//		라) private : 외부에서는 접근 불가, 내부멤버(멤버핃드, 멤버메서드) 끼리는 접근 가능
	//				
	...
	// 3. 멤버메서드 : 동작, 기능, 실행
	//	   - 해당 메서드를 호출(즉, 실행)하면 해당 메서드안에 존재한 내용이 실행됨
	//     - 호출해서 내용이 실행 된 후에는 호출한 곳으로 되돌아간다.
	//     - 메서드는 다른 메서드를 호출할 수 있음
	//	1) 구성 : [접근제한자] [메서드종류] 봔환형 메서드이름([인자=매개변수]){ 실행할 내용 }
	//	  가) 접근제한자 : public, protected, 생략 private ; 
	//	  나) 메서드종류 : 일반메서드 이부분을 생략한다.
	//				   특정 메서드에 해당하는 예약어를 사용(static)
	//	  다) **반환형  : 해당 메서드가 실행이 끝나면 호출한 곳으로 되돌아 갈때
	//				   함께 가지고 가는 결과의 자료형
	// 				   만약에 결과를 가지고 가지 않으면 void 라는 예약어를 사용한다.
	//  2) 메서드 이름 : 변수와 같음, 첫글자 소문자, 두 단어 이상일 떄 첫글자는 대문자
	//				  숫자는 중간이나 뒤에 사용, 공백은 사용 불가
	//  3) ** 인자 = 매개변수 : 보통은 실행할 때 필요한 정보를 외분에서 받기 위한 통로
	//     				 생략 가능하고 여러개 사용 가능
	//  4) ** 오버로딩(중복정의) : 메서드 이름이 같으나 인자 수나 인자 자료형이 다르면 다른 메서드로 판단한다.
	//					   즉, 인자의 수나 자료형이 다르면 같은 이름의 메서드가 여러개 존재할 수 있다.
	//  5) ** getter() : 호출하는 입장에서 메서드를 이용해서 정보를 가져오는 메서드
	//	   ** settet() : 호출하는 입장에서 메서드를 이용해서 정보를 변경하는 메서드
	...
	//		클래스     참조변수(변수이름)   	 new예약어 		생성자
	
 	//    new 예약어를 사용하면 무조건 heap 메모리에 사용역역(저장영역)을 만든다.
	
	// 5. 클래스를 객체로 만든 후 멤버필드와 멤버메서드에게 접근하는 방법 
	//	  참조변수.멤버필드,	참조변수.멤버메서드(),		(단, 접근제한자를 확인)
	
	// 6. static 과 instance : 멤버필드, 멤버메서드에 공통으로 해당하는 사항
	//   1) instance(인스터스) 필드, 메서드 : 객체가 생성될 때 같이 생성되는 필드와 메서드
